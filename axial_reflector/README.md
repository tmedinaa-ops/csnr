...
4. Sweep the thickness to find where the worth saturates. `reflector_sweep.py` runs k-eff for the baseline and a set of added-radial-Be and axial-cap thicknesses, and reports worth (pcm), beryllium mass (kg), and the horizon at each. This is the measured version of the worth-vs-thickness schematic in ../depletion_7yr/reflector_worth_schematic.png: the first few cm are worth the most, past ~10 to 15 cm you are buying mass not reactivity, and the plot tells you the cheapest point on the curve that reaches the horizon you need. It has a snap.py hook for building the variants cleanly and a generic outer-shell fallback that works without snap.py; verify the beryllium landed where you expect with a plots.xml before trusting the numbers.
...
Files: `leakage_split.py` (axial vs radial leakage tally), `Axial_Reflector_Variant_Guide.md` (geometry change + depletion recipe), `reflector_sweep.py` (measured worth vs thickness and mass), `Pseudo_Breeder_Analysis.md` (the fertile-blanket thought experiment).
